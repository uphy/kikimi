...
struct SessionMetaTests {
    ...
    // MARK: profileId (docs/design/41-meeting-profiles.md §2.3)

    @Test("decodes a meta.json with no profile_id key at all as profileId == nil, for backward compatibility")
    func decodesMissingProfileIdKeyAsNil() throws {
        // `sampleJSON` predates design 41 and has no `profile_id` key at all.
        let decoder = SessionJSONCoding.makeDecoder()
        let meta = try decoder.decode(SessionMeta.self, from: Data(Self.sampleJSON.utf8))
    ...
    @Test("round-trips a Draft session seeded from a profile, encoding profileId as the profile_id key")
    func roundTripsProfileId() throws {
        let original = SessionMeta(
            id: "2026-07-02T09-00-00_deadbeef",
            title: "",
            titleAutoGenerated: true,
        ...
        #expect(decoded == original)
        #expect(decoded.profileId == "daily-scrum")

        let object = try #require(JSONSerialization.jsonObject(with: data) as? [String: Any])
        #expect(object["profile_id"] as? String == "daily-scrum")
    }

    @Test("omits the profile_id key entirely when profileId is nil (a plain \"+ 新規\"/複製 draft stays byte-for-byte unchanged)")
    func omitsProfileIdKeyWhenNil() throws {
        let original = SessionMeta(
            id: "2026-07-02T09-00-00_deadbeef",
            title: "",
            titleAutoGenerated: true,
        ...
        #expect(original.profileId == nil)

        let data = try SessionJSONCoding.makeEncoder().encode(original)
        let object = try #require(JSONSerialization.jsonObject(with: data) as? [String: Any])

        #expect(object["profile_id"] == nil)
        #expect(object.keys.contains("profile_id") == false)
    }

    @Test("profileId and basedOnSession are independent fields -- a based_on-seeded session round-trips with profileId nil")
    func basedOnSessionAndProfileIdAreIndependentFields() throws {
        let original = SessionMeta(
            id: "2026-07-02T09-00-00_deadbeef",
            title: "",
            titleAutoGenerated: true,
            ...
            endedAt: nil,
            durationMs: 0,
            basedOnSession: "2026-06-24T14-30-00_x9y8z7w6",
            profileId: nil,
            segmentCount: 0,
            refinedCount: 0,
            appVersion: "0.1.0"
        ...
        #expect(decoded.basedOnSession == "2026-06-24T14-30-00_x9y8z7w6")
