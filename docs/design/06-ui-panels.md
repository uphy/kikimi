...
| Markdown/リッチテキスト表示 | WKWebView + CodeMirror（`editor-web`） | **WKWebView は使わない**。Prep タブは `NSTextView`（プレインテキスト、kikimi.md 10章で明記）、Summary/Watchers タブは SwiftUI の `AttributedString`/Markdown レンダリングで表示（kikimi.md 13章「MVP は SwiftUI Markdown プレビューで十分な可能性」） | 実装コスト削減が kikimi.md 自身の判断として明記されている |
